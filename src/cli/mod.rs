...
impl Args {
    ...
    pub fn validate(&self) -> Result<(), ConversionError> {
        ...
        if out_dir.exists() && !out_dir.is_dir() {
            return Err(ConversionError::InvalidFormat(
                format!("Output path '{}' exists but is not a directory", self.output_dir())
            ));
        }
        ...
        if let Some(ref pwd) = self.password {
            if pwd.is_empty() {
                ...
                eprintln!("Warning: Empty password provided. If the PFX file requires a password, conversion might fail.");
            }
        }
